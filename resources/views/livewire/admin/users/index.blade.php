            ...
            <tr>
                ...
                <td>
                    @if (! empty($user->invite_token))
                        <small class="dark:text-gray-300">Invited<br> {{ date('d/m/Y H:i', strtotime($user->invited_at)) }}</small>
                    @else
                ...
                <td>
                    <div class="flex space-x-2">
                            ...
                            @if(can('view_users_profiles'))
                                <a href="{{ route('admin.users.show', ['user' => $user->id]) }}">{{ __('Profile') }}</a>
                            @endif

                            @if(can('edit_users'))
                                <a href="{{ route('admin.users.edit', ['user' => $user->id]) }}">{{ __('Edit') }}</a>
                            ...
                            @if(can('add_users'))
                                @if (!empty($user->invite_token))
                                    <x-modal>
                                            ...
                                            @if($sentEmail === false)
                                                <x-slot name="title">{{ __('Send') }} {{ $user->name }} {{ __('another invite email.') }}</x-slot>
