<div>
    <div class="card">
        ...
        <x-form wire:submit.prevent="" method="put">
            ...
            <x-form.input wire:model="name" label="{{ __('Name') }}" name='name' required></x-form.input>
            <x-form.input wire:model="email" label="{{ __('Email') }}" name='email' required></x-form.input>
            <x-form.input wire:model="image" type="file" label="{{ __('Image') }}" name='image'></x-form.input>
            @if ($image)
                {{ __('Photo Preview') }}:
